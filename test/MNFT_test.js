...
describe("MNFT Contract", () => {
  const setup = async ({ maxSupply = 10000 }) => {
    const [owner] = await ethers.getSigners();
    const MNFT = await ethers.getContractFactory("MNFT");
    const deployed = await MNFT.deploy(maxSupply);
    return {
      owner,
      deployed,
    };
  ...
  describe("Minting", () => {
    it("Mints a new token and assigns it to owner", async () => {
      ...
      await deployed.selfMint();
      const ownerOfMinted = await deployed.ownerOf(0);
      expect(ownerOfMinted).to.equal(owner.address);
    });
  });
  describe("Has a minting limit", () => {
    it("Dont mint pass the maxSupply limit", async () => {
      const maxSupply = 2;
      const { deployed } = await setup({ maxSupply });
      await Promise.all([deployed.selfMint(), deployed.selfMint()]);
      // Assert the last minting
      await expect(deployed.selfMint()).to.be.revertedWith(
        "No more Matterblocks left."
      );
    });
  });
  describe("Token URI", () => {
